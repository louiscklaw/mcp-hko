...
export async function makeHhotRequest({ station, year, month, day, hour, rformat = 'json' }) {
 ...
 - Data rows: Actual tide data
    `,
        parameters: z.object({
            ...
            year: z.number().describe('Year between 2022-2024'),
            month: z.number().optional().describe('Optional month (1-12)'),
            day: z.number().optional().describe('Optional day of month (1-31)'),
            hour: z.number().optional().describe('Optional hour (0-23)'),
            rformat: z
                .string()
                .default('json')
                .describe("Output format (only 'json' supported)")
        }),
        execute: async (args) => {
